...
def generate_column_rules(df: pd.DataFrame,
                          skip_index: bool = True,
                          level: int = 0,
                          left_trim: TrimSpec = True,
                          right_trim: TrimSpec = True) -> List[RuleSpecifier]:
...
def generate_partition_rules(df: pd.DataFrame,
                             skip_index: bool = False,
                             level: int = 0) -> List[RuleSpecifier]:
...
def write_table(styler: style.Styler,
                ...
                **kwargs) -> str:
    out_file = resolve_directory(DirType.TABLES, subdir) / f'{filename}.tex'
    ...
    if midrules is not None:
        if not isinstance(midrules, list):
            midrules = [midrules]
        rules = filter(lambda x: x[0] >= 0,
                       sorted([_rule_from_spec(midrules) for midrules in midrules],
                              key=lambda x: x[0]))
    ...
    with open(out_file, 'w', encoding='utf-8') as fh:
        ...
        fh.write(f"% This file is automatically generated by {os.path.basename(sys.argv[0])}\n")
        fh.write("\n")
        if standalone:
            if isinstance(standalone, str):
                fh.write("\\documentclass[border=0.2cm,%s]{standalone}\n\n"
                         % (standalone))
            else:
                fh.write("\\documentclass[border=0.2cm]{standalone}\n\n")
            fh.write("\\usepackage{booktabs}\n")
            fh.write("\\usepackage[table]{xcolor}\n")
            if siunitx:
                fh.write("\\usepackage{siunitx}\n")
                fh.write("\\usepackage{etoolbox}\n")
                fh.write("\\robustify\\itshape\n")
                fh.write("\\robustify\\bfseries\n")
        ...
        if colsep:
            ...
            fh.write('\\renewcommand{\\tabcolsep}{' + colsep + "}\n")
        fh.write(table)
        if colsep:
            fh.write('\\renewcommand{\\tabcolsep}{\\oldtabcolsep' + col_sep_prefix + '}\n')
        if standalone:
